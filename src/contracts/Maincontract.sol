...
contract Maincontract {
    ...
    // Variables
    string public name = "Main Contract";
    ...
    event SwitchMarket(address owner, uint256 amount, address depositTo);
    ...
    function deposit(
        ...
        if (amountDeposited > 0) {
            switchMarket(_compAPY, _aaveAPY);
        }
    ...
    function switchMarket(uint256 _compAPY, uint256 _aaveAPY) public onlyOwner {
        ...
        if ((_compAPY > _aaveAPY) && (locationOfFunds != address(cDai))) {
            ...
            emit SwitchMarket(msg.sender, amountDeposited, locationOfFunds);
        } else if (
            (_aaveAPY > _compAPY) &&
            (locationOfFunds != address(aaveLendingPool))
        ) {
